...
public function like($like, $value = NULL, $position = 'both')
	...
	{
		$this->trigger_events('get_user');

		

		$this->limit(1);
		$this->order_by($this->tables['users'].'.email', 'desc');
		$this->where($this->tables['users'].'.email', $username);
	...
	**/
	...
	protected function _prepare_ip($ip_address) {
        ...
        public function get_menu_group($id_grupo){
            ...
            $this->db->join('admin_menu_groups', 'admin_menu_groups.id_menu= admin_menu.id_menu');
            $this->db->where('admin_menu_groups.id_group',$id_grupo);
            $query= $this->db->get();
            //$query = $this->db->query("SELECT * FROM admin_menu INNER JOIN admin_menu_groups  where group_id = ? order by name;",array($id_grupo));                
            //echo $this->db->last_query();
            return $query->result();                
